...
<template>
  <app-section title="Дерево">
    ...
    <template #example1>
      <p>
        Представьте себе блог, где пользователи могут оставлять комментарии к постам, а также
        отвечать на комментарии других пользователей. Дерево может быть использовано для
        представления этой иерархии комментариев.
      </p>
    ...
    <template #example2>
      <p>
        В этом примере показана реализация файловой структуры, где есть папки и файлы. Папки можно
        раскрывать и сворачивать, чтобы просмотреть их содержимое. Файлы отображаются внутри папок,
        и вы можете видеть их размеры. Представим также ситуацию, что файл можно открыть и изменить
        его содержимое
      </p>
    ...
    <template #pros> Когда иерархическая структура дерево и нужны связи потомок/родитель </template>
