<template>
  <p>
    Стек — это структура данных, работающая по принципу LIFO (Last In, First Out), что означает, что последний добавленный элемент будет первым извлечен.
    В данном контексте стек используется для управления модальными окнами. Это позволяет открывать и закрывать модальные окна в определенном порядке,
    обеспечивая корректное поведение при нажатии на крестик или клавишу Escape, чтобы закрыть только последнее открытое модальное окно.
    Ниже представлен пример с модальным окном и его подтверждением удаления.
  </p>
